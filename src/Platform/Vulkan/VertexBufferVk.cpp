...
void VertexBufferVk::Bind(VkCommandBuffer cmdBuffer, uint64_t offset) const {
    VkDeviceSize bufferOffset = offset;
    vkCmdBindVertexBuffers(cmdBuffer, 0, 1, m_Buffer.bufferPtr(), &bufferOffset);
}

void VertexBufferVk::UploadData(const void *data, uint64_t bytes) {
    ...
    vk::CommandBuffers transferCmdBuffers(m_Device, m_Device.TransferPool()->data());
    {
        ...
        m_Buffer = DeviceBuffer(&m_Device, data, bytes, transferCmdBuffer, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
