...
public:
    ...
    vk::DescriptorSets createDescriptorSets(const vk::DescriptorPool& pool, const std::vector<VkDescriptorSetLayout>& layouts) const {
    ...
    vk::CommandBuffer createCommandBuffer(const vk::CommandPool& pool) const { return vk::CommandBuffer(m_LogicalDevice, pool.data()); }
    ...
    vk::Image
    createImage(const std::set<uint32_t>& queueIndices, VkExtent2D extent, uint32_t mipLevels, VkSampleCountFlagBits samples, VkFormat format,
    ...
    vk::ImageView createImageView(const vk::Image& image, VkImageAspectFlags aspectFlags = VK_IMAGE_ASPECT_COLOR_BIT) const {
       return vk::ImageView(m_LogicalDevice, image, aspectFlags);
    }

    vk::ImageView
    createImageView(VkImage image, uint32_t mipLevels, VkFormat format, VkImageAspectFlags aspectFlags = VK_IMAGE_ASPECT_COLOR_BIT) const {
    ...
    vk::Framebuffer createFramebuffer(const VkExtent2D& extent, VkRenderPass renderPass, const std::vector<VkImageView>& attachments) const {
    ...
    vk::PipelineLayout createGraphicsPipelineLayout(const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts) const {
    ...
    std::vector<VkSurfaceFormatKHR> getSurfaceFormats() const {
       ...
       if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_PhysicalDevice, m_Surface->data(), &formatCount, nullptr) != VK_SUCCESS)
          throw std::runtime_error("Could not query surface for supported formats");
       std::vector<VkSurfaceFormatKHR> formats(formatCount);
       if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_PhysicalDevice, m_Surface->data(), &formatCount, formats.data()) != VK_SUCCESS)
          throw std::runtime_error("Could not query surface for supported formats");
       return formats;
    }
    ...
    std::vector<VkPresentModeKHR> getSurfacePresentModes() const {
       ...
       if (vkGetPhysicalDeviceSurfacePresentModesKHR(m_PhysicalDevice, m_Surface->data(), &presentModeCount, nullptr) != VK_SUCCESS)
       ...
       if (vkGetPhysicalDeviceSurfacePresentModesKHR(m_PhysicalDevice, m_Surface->data(), &presentModeCount, modes.data()) != VK_SUCCESS)
          throw std::runtime_error("Could not query surface for supported present modes");
       return modes;
    }
