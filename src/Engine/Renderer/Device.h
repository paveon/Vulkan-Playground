...
//        return VK_SAMPLE_COUNT_1_BIT;
    ...
    auto createDescriptorSets(const vk::DescriptorPool &pool, const std::vector<VkDescriptorSetLayout> &layouts,
                              const std::vector<uint32_t>& variableCounts) -> vk::DescriptorSets * {
       m_DescriptorSets.emplace_back(
               std::make_unique<vk::DescriptorSets>(m_LogicalDevice.data(), pool.data(), layouts, variableCounts));
    ...
    auto createDescriptorSetLayout(const std::vector<
            std::pair<VkDescriptorSetLayoutBinding, VkDescriptorBindingFlagsEXT>> &bindings)
    ...
    auto enabledFeatures() const -> const VkPhysicalDeviceFeatures & {
       return m_LogicalDevice.enabledFeatures();
    }
