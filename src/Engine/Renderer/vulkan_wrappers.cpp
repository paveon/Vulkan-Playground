...
namespace vk {
    const std::array<const char *, 4> LogicalDevice::s_RequiredExtensions = {
    ...
    LogicalDevice::LogicalDevice(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface) :
       ...
       supportedFeatures.pNext = &supportedVk12Features;
       vkGetPhysicalDeviceFeatures2(m_Physical, &supportedFeatures);
       VkPhysicalDeviceVulkan12Features vulkan12Features{};
//       if (!supportedVk12Features.descriptorIndexing) {
//          throw std::runtime_error("[Vulkan] Descriptor Indexing feature is not supported by selected device");
//       }
    ...
    void ShaderModule::ExtractIO(const spirv_cross::CompilerGLSL &compiler,
       ...
       for (const auto &resource: resources) {
          ...
          Log() << "[ShaderModule (" << m_Name << ")] Shader input: " << resource.name << std::endl;
       }
    ...
    ShaderModule::ShaderModule(VkDevice device, const std::string &filename) : m_Filepath(filename), m_Device(device) {
       ...
       auto pos = m_Filepath.rfind('/');
       m_Name = m_Filepath.substr(pos > 0 ? pos + 1 : pos);
...
//                    binding.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
          ...
          Log() << "[ShaderModule (" << m_Name << ")] Sampler: "
                    << resource.name << ", Set: " << setIdx << ", Binding: " << bindingIdx << std::endl;
       }

       m_CodeSize = shaderCode.size() * sizeof(uint32_t);
       Log() << "[Vulkan] Loaded shader '" << m_Name << "' bytecode, bytes: " << m_CodeSize << std::endl;
    ...
    Instance::Instance(bool enableValidation) {
       Log() << currentTime() << "[Vulkan][Instance] Constructing" << std::endl;
       auto requiredLayers = ArrayToVector(s_RequiredLayers);

       if (enableValidation) {
          Log() << currentTime() << "[Vulkan][Instance] Enabling validation layers" << std::endl;
          requiredLayers.insert(requiredLayers.end(), s_ValidationLayers.begin(), s_ValidationLayers.end());
       }

       const auto layers = checkLayerSupport(requiredLayers, ArrayToVector(s_OptionalLayers));
       for (const auto& layer : layers) {
          Log() << currentTime() << "[Vulkan][Instance] Enabled layer: " << layer << std::endl;
       }
       ...
       for (const auto& ext : extensions) {
          Log() << currentTime() << "[Vulkan][Instance] Enabled extension: " << ext << std::endl;
       }
       ...
       vk::CheckResult(vkCreateInstance(&createInfo, nullptr, &m_Instance),
                       "[Vulkan][Instance] Call to 'vkCreateInstance' failed");
    ...
    void Instance::setupDebugMessenger() {
       ...
       vk::CheckResult(CreateDebugUtilsMessengerEXT(m_Instance, &createInfo, nullptr, &m_DebugMessenger),
                       "[Vulkan][Instance] Call to 'CreateDebugUtilsMessengerEXT' failed");
    }
    ...
    DescriptorSetLayout::DescriptorSetLayout(
            VkDevice device,
            const std::vector<std::pair<VkDescriptorSetLayoutBinding, VkDescriptorBindingFlagsEXT>> &bindings)
            : m_Device(device) {

       std::vector<VkDescriptorBindingFlagsEXT> bindingFlags(bindings.size());
       for (size_t i = 0; i < bindings.size(); i++) {
          if (bindings[i].first.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
             bindingFlags[i] = bindings[i].second;
             if ((bindings[i].second & VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT) != 0) {
                m_LayoutFlags |= VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT;
             }
...
//                     | VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT;
       ...
       std::vector<VkDescriptorSetLayoutBinding> rawBindings(bindings.size());
       std::transform(bindings.begin(), bindings.end(), rawBindings.begin(), [](const auto& binding) {
       ...
       layoutInfo.pBindings = rawBindings.data();
       layoutInfo.pNext = &ext;
       layoutInfo.flags = m_LayoutFlags;

       CheckResult(vkCreateDescriptorSetLayout(m_Device, &layoutInfo, nullptr, &m_Layout),
