...
//   if (queueIndex(QueueFamily::GRAPHICS) != queueIndex(QueueFamily::PRESENT)) {
//      std::array<uint32_t, 2> indices = {
//              queueIndex(QueueFamily::GRAPHICS),
//              queueIndex(QueueFamily::PRESENT)
//      };
//
...
//   }
    ...
    void ChangeLayout(const CommandBuffer& cmdBuffer, VkFormat format, VkImageLayout newLayout);
...
private:
    ...
    void Release() noexcept { if (m_Memory) vkFreeMemory(m_Device, m_Memory, nullptr); }
...
public:
    ...
    uint32_t MipLevels() const { return std::floor(std::log2(std::max(Width, Height))) + 1; }
};
